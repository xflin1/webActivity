...
app.config(function ($routeProvider) {
    $routeProvider.when('/',{templateUrl: 'login.html',controller:'Login',reloadOnSearch: false})
        .when('/register',{templateUrl: 'register.html',controller:'Register',reloadOnSearch: false})
        .when('/login',{templateUrl: 'login.html',controller:'Login',reloadOnSearch: false})
        .when('/account',{templateUrl: 'account.html',controller:'Account',reloadOnSearch: false})
        .when('/myActivity',{templateUrl: 'myActivity.html',controller:'MyActivity',reloadOnSearch: false})
        .when('/publishActivity',{templateUrl: 'publishActivity.html',controller:'PublishActivity',reloadOnSearch: false})
        .when('/activityManage',{templateUrl: 'activityManage.html',controller:'MyActivity',reloadOnSearch: false})
...
app.controller('Register',function ($scope,$http) {//不要忘记$http的定义！！！！

    $scope.isPerson= true;
    $scope.cb_checked=false;
    ...
    $scope.changeNav=function () {
        ...
        $("#registerNav").toggleClass("active");
    }();
    ...
    $scope.register_p=function (user) {
        ...
        error(function() {
            ...
            console.log("Register_p:error");
        });

    };
    $scope.register_c=function (company) {

        var url="/api/register/vendor";
        $http.post(url,company)    //这里没有分号！！
            .success(function(data){
                if(data.code===0){
                    window.location.href='./#/login';
                    alert("企业注册成功！");
...
app.controller('Account',function ($scope,$http,$route) {
    //用来存储用户信息
    $scope.uploadImage="";
    $scope.edit=true;
    $scope.info={};
    ...
    $scope.ShowTarget=function () {
        ...
        if(!$scope.showTarget){
            var url="/service/admin/list/vendor";//publish/target
            $http.post(url)    //这里没有分号！！
                .success(function(data){
                    if(data.code===0){
                        //判断获取的vendorlist列表是否为空
                        if(data.list.length){
                            $scope.vendorTemp.vid=data.list[0].id;
                            var url2="/service/publish/target";
                            $http.post(url2,$scope.vendorTemp)
                                .success(function(data){
                                    if(data.code===0){
                                        if(data.target.length)
                                        {
                                            $scope.targetEmpty=false;
                                ...
                                .error(function(){
                                    console.log("getTarget:error");
                                });
                        }
                        else{
                            $scope.vendorTemp.vid="null";
                        }
                        // alert(data.list[0].id+","+data.list[0].name);

                        //alert(target[0].id+","+target[0].name);

                    }else{
                        alert("获取vendorlist失败！");
                    }
    ...
    $scope.getUserInfo=function(){
        ...
        $http.post(url)    //这里没有分号！！
            .success(function(data){
                if(data.code===0){
                    if(data.user.extattrs==undefined)
                    {
                        $scope.info.extattrs.vendorName="该内容尚未定义";
                        $scope.info.extattrs.realName="该内容尚未定义";
                        $scope.info.extattrs.address="该内容尚未定义";
                        $scope.mySelf="该内容尚未定义";
                    }
                    else{
                        //公司名
                        if(data.user.extattrs.vendorName==undefined){
                            $scope.info.extattrs.vendorName="该内容尚未定义";
                        }
                        else $scope.info.extattrs.vendorName=data.user.extattrs.vendorName;
                        //真实姓名
                        if(data.user.extattrs.realName==undefined){
                            $scope.info.extattrs.realName="该内容尚未定义";
                        }
                        else $scope.info.extattrs.realName=data.user.extattrs.realName;
                        //地址
                        if(data.user.extattrs.address==undefined){
                            $scope.info.extattrs.address="该内容尚未定义";
                        }
                        else $scope.info.extattrs.address=data.user.extattrs.address;
                        //自定义
                        if(data.user.extattrs.mySelf==undefined){
                            $scope.mySelf="该内容尚未定义";
                        }
                        else $scope.mySelf=data.user.extattrs.mySelf;
                    }
                    //phone
                    if(data.user.phone==undefined){
                        $scope.info.phone="该内容尚未定义";
                    }
                    else $scope.info.phone=data.user.phone;
                    //weixin
                    if(data.user.weixin==undefined){
                        $scope.info.weixin="该内容尚未定义";
                    }
                    else $scope.info.weixin=data.user.weixin;
                    //photoId
                    $scope.info.photoId=data.user.photoId;
                    // alert($scope.info.photoId);
    ...
    $scope.display_zlsz=function () {
        $scope.zlsz=true;
        $scope.xgmm=false;
        $scope.qxsz=false;
    };
    $scope.display_xgmm=function () {
        $scope.zlsz=false;
        $scope.xgmm=true;
        $scope.qxsz=false;
    };
    $scope.display_qxsz=function () {
        $scope.zlsz=false;
        $scope.xgmm=false;
        $scope.qxsz=true;
    };
    ...
    // 提交密码修改请求
    $scope.changePwd=function (change) {

        var url="/api/user/info";
        $http.post(url)    //这里没有分号！！
            .success(function(data,status){
                if(data.code===0){
                    change.name=data.user.nickname;//先获取当前用户name字段的值
                    var url2="/api/user/password"; //再发送post请求
                    $http.post(url2,change)    //因为post请求是异步操作，所以这个post要放在里层
                        .success(function(data){
                            if(data.code===0){
                                alert("修改密码成功，请重新登录！");
                                window.location.href="./#/login";
        ...
        error(function(data, status, headers, config) {
            // called asynchronously if an error occurs
            // or server returns response with an error status.
            console.log("ChangePwd:error1");
        });
    };
    //修改个人信息
    $scope.submitChange=function (info) {

        var url="/api/user/info";
        $http.post(url)    //这里没有分号！！
            .success(function(data){
                if(data.code===0){
                    info.photoId=data.user.photoId;
                    url="/api/user/update";
                    $http.post(url,info)
                        .success(function (data) {
                            if(data.code===0) {
                        ...
                        .error(function () {
                            console.log("SubmitChange:error!");
                        });
    ...
    $scope.uploadImg=function(file) {
        ...
        var inputValue = $("#uploadImage").val();
        $scope.saveDisabled=true;
        $scope.uploadBtn=true;
        // $("#uploadSave").attr("disabled", true);
        // $("#uploadCancel").attr("disabled", true);

        //通过input的files对象就可以读取到用户所选择的文件
        var imgurl = window.URL.createObjectURL(file.files[0]);
        // $("#reuploadImageButton0").css("display", "block");
        // $("#reuploadImageButton0").attr("disabled", true);
        $("#imgPreview").html(
            '<img id="imagePreview" src="'
            + imgurl + '" class="img-responsive img-thumbnail" ' +
        ...
        var imgType = 'image/' + imgName.substring(imgName.lastIndexOf('.') + 1);
        //文件每片分割2048000B，计算分割详情
        var imgChunkSize = 2048000; //以2048000B为一个分片
        var imgChunks = Math.ceil(imgSize / imgChunkSize); //总片数
        var imgChunkNumber = 0; //上传分片的序列
        //创建md5对象（基于SparkMD5）
        var imgSpark = new SparkMD5.ArrayBuffer();
        var imgMd5;
        var imgReader = new FileReader();
        var imgFilePart = null;

        //获取第一个分片的md5
        function getImgMd5() {

            var end = imgSize <= imgChunkSize ? imgSize : imgChunkSize;
            var imgFirstReader = new FileReader;
            var imgFileFirstPart = imgFile.slice(0, end);
            imgFirstReader.onload = function(e) {
                imgSpark.append(e.target.result);
                imgMd5 = imgSpark.end();
                console.log(imgMd5);

                //调用uploadStatus判断服务器是否存在文件
                $.get("/upload/status", {
                    file: imgMd5
                }, function(data) {
                    if(data.code == 0) {
                        if(data.size == 0) {
                            ...
                            //开始读取文件
                            loadNextImg();
                        } else if(data.size >= imgSize) {
                            console.log("文件已存在，获取文件ID...");
                            //已存在文件
                            //上传0个分片，只获取图片文件ID
                            var formData = new FormData();
                            formData.append("file", imgFileFirstPart);
                            formData.append("type", imgType);
                            formData.append("_chunkNumber", '0');
                            formData.append("_chunkSize", '2048000');
                            formData.append("_currentChunkSize", '0');
                            formData.append("_totalSize", imgSize);
                            formData.append("md5", imgMd5);
                            ...
                            $.post(
                                ...
                                if(data.code == 0) {
                                    console.log("成功获取文件ID！" + "文件ID: " + data.fid);
                                    //获取图片的文件ID，保存时存入数据库
                                    $scope.uploadImage = data.fid;
                                    $scope.saveDisabled = false;
                                    $scope.uploadBtn = false;
                                    alert("成功上传图片！");
                                    console.log($scope.saveDisabled);


                                    // $("#uploadSave").attr("disabled", false);
                                    // $("#uploadCancel").attr("disabled", false);
                                    // $("#reuploadImageButton0").attr("disabled", false);

                                    //重新绑定图片上传的onchange函数
                                    $("#uploadImage")
                                        .replaceWith('<input type="file" accept="image/*" ' +
                                            'name="uploadImage" id="uploadImage" ' +
                                            'onchange="javascript:uploadImg(this);" ' +
                                            'style="display:none" />');
                                } else {
                                    console.log("获取文件ID失败！");
                                }
                            });
                            ...
                            //续传
                            console.log("开始续传...");
                            imgChunkNumber = Math.floor(data.size / imgChunkSize);
                            //开始读取文件
                            loadNextImg();
                        }
                    } else {
            ...
            imgFirstReader.readAsArrayBuffer(blobSlice.call(imgFile, 0, end));
        }
        ...
        imgReader.onload = function(e) {
            var currentChunkSize = (imgSize - imgChunkNumber * imgChunkSize) >= imgChunkSize ? imgChunkSize : (imgSize - imgChunkNumber * imgChunkSize);
            //上传分片
            console.log("上传分片" + imgChunkNumber);

            var formData = new FormData();
            formData.append("file", imgFilePart);
            formData.append("type", imgType);
            formData.append("_chunkNumber", imgChunkNumber);
            formData.append("_chunkSize", '2048000');
            formData.append("_currentChunkSize", currentChunkSize);
            formData.append("_totalSize", imgSize);
            formData.append("md5", imgMd5);
            ...
            $.post({
                ...
                if(data.code == 0) {
                    imgChunkNumber++;
                    //如果文件处理完成，计算MD5，如果还有分片继续处理
                    if(imgChunkNumber < imgChunks) {
                        loadNextImg();
                    } else {
                        console.log("文件上传完成！");
                        //获取图片的文件ID，保存时存入数据库
        ...
        function loadNextImg() {
            var start = imgChunkNumber * imgChunkSize;
            var end = (start + imgChunkSize) >= imgSize ? imgSize : (start + imgChunkSize);
            imgReader.readAsBinaryString(blobSlice.call(imgFile, start, end));
            imgFilePart = imgFile.slice(start, end);
        }
    ...
    $scope.saveImg=function() {
        ...
        if($scope.uploadImage == null) {
            window.alert("请上传图片！");
        } else {
            var url="/api/user/update";
            var img={};
            img.photoId=$scope.uploadImage;
            $http.post(url,img)
                .success(function(data){
                    if(data.code===0){
                        $scope.saveDisabled=true;
    ...
    $scope.resetUpload=function () {
        // $("#reuploadImageButton0").css("display", "none");
        ...
        $("#imgPreview")
            .html('<button type="button" class="btn btn-default" ' +
                'name="uploadImageButton" id="uploadImageButton" ' +
                'onclick="javascript:$(' + "'#uploadImage'" + ').click();" ' +
                'style="height: 200px;" /></button>');
        $("#uploadImageButton")
            .html('<h1 class="text-center" ' +
                'style="font-family: ' + "'黑体'" + ';">上传图片</h1>');

    };

    $scope.clickStatus=function (x) {
        x.selected = !x.selected;
        //如果选中某一个目标则添加到temp变量中
        if(x.selected)
        {
            //如果当前选中的目标不在temp中则添加进去
            if($scope.temp.indexOf(x.id)<0)
                $scope.temp+=x.id+"("+x.name+")"+",";
        }
        else
        {
            $scope.temp=$scope.temp.replace(x.id+"("+x.name+")"+",","");
        }
        // alert($scope.temp);
    };
    //修改自定义后的显示
    $scope.submitChecked=function () {
        //如果获取到的发布目标为空则只需隐藏显示
        if($scope.targetEmpty)
            $scope.show=false;
        else{
            $scope.mySelf=$scope.temp.substr(0,$scope.temp.length-1);
            $scope.show=false;
            var url="/api/user/info";
            $http.post(url)    //这里没有分号！！
                .success(function(data){
                    if(data.code===0){
                        var tempData=data.user;//先获取当前用户信息
                        //如果extattrs不存在则为它分配空间
                        if(tempData.extattrs==undefined)
                            tempData.extattrs={};
                        tempData.extattrs.mySelf=$scope.mySelf;//添加自定义信息
                        var url2="/api/user/update"; //再发送post请求
                        $http.post(url2,tempData)    //因为post请求是异步操作，所以这个post要放在里层
                            .success(function(data){
                                if(data.code===0){
                                    alert("修改自定义成功！");
                                }else{
                        ...
                        error(function(data, status, headers, config) {
                            // called asynchronously if an error occurs
                            // or server returns response with an error status.
                            console.log("submitChecked:error2");
                        });
                    }else{
                        alert("获取userinfo失败！");
                    }
            ...
            error(function(data, status, headers, config) {
                // called asynchronously if an error occurs
                // or server returns response with an error status.
                console.log("submitChecked:error1");
            });
        }
...
app.controller('MyActivity',function ($scope,$http,$location,$rootScope) {

    //判断活动数量是否为零
    $scope.activityNum=1;
    //判断是否点击了编辑标题按钮
    $scope.editTitleClicked=false;
    $scope.editContentClicked=false;
    //存放编辑后的内容
    $scope.edit={};
    ...
    $scope.changeNav=function () {
        //先去掉其他已经被点击过的导航的颜色
        $(".active").toggleClass("active");
        $("#myInfoNav").toggleClass("active");
    }();

    $scope.getActivity=function () {
        var temp={};
        var url="/service/admin/list/vendor";
        // 导航栏变化显示
        $(".active").toggleClass("active");
        $("#myInfoNav").toggleClass("active");
        $http.post(url)    //这里没有分号！！
            .success(function(data){
                if(data.code===0){
                    if(data.list.length) {
                        temp.vid=data.list[0].id;
                        temp.page=0;
                        temp.limit=20;
                        var url2="/api/service/admin/list/data";
                        $http.post(url2,temp)
                            .success(function (data) {
                                $scope.activityData=data.list;
                                $scope.activityNum=data.list.length;
                            }).
                        error(function(){
                            console.log("GetActivity:error");
                        });
                    }
                    else $scope.activityNum=0;
    ...
    $scope.getuserInfo=function() {
        ...
        $http.post(url)    //这里没有分号！！
            .success(function(data){
                if(data.code===0){
                    //alert(data.user.extattrs.realName);
                    $rootScope.publishUser=data.user;
                }else{
                    alert("获取userinfo失败！");
                }
    ...
    $scope.activityManage=function (x) {
        $rootScope.manageData=x;
        //$scope.getuserInfo();
        $location.path("/activityManage");
    };
    $scope.editTitle=function () {
        $scope.editTitleClicked=true;
        $scope.edit.title=$rootScope.manageData.name;
    };
    //确认修改活动标题
    $scope.editTitleConfirm=function (edit) {
        $scope.editTitleClicked=false;
        var temp={};
        temp.sd=$rootScope.manageData.id;
        var url="/api/service/admin/data";
        $http.post(url,temp)
            .success(function (data) {
                if(data.code==0){
                    temp=data.data;
                    temp.name=edit.title;
                    temp.sd=temp.id;
                    var url2="/api/service/admin/update";
                    $http.post(url2,temp)
                        .success(function (data) {
                            if(data.code==0){
                                alert("修改标题成功!");
                                $location.path("/myActivity");
                            }
                            else{
                                alert("修改失败!");
                            }
                        ...
                        .error(function () {
                            console.log("error:post /service/admin/update");
                        });
                }
            })
            .error(function () {
                console.log("error:post /service/admin/data");
            });
    };
    $scope.editContent=function () {
        $scope.editContentClicked=true;
        $scope.edit.content=$rootScope.manageData.content;
    };
    //修改活动内容
    $scope.editContentConfirm=function (edit) {
        $scope.editContentClicked=false;
        var temp={};
        temp.sd=$rootScope.manageData.id;
        var url="/api/service/admin/data";
...
app.controller('PublishActivity',function ($scope,$http,$location) {
            ...
            $http.post(url)    //这里没有分号！！
                .success(function(data){
                    if(data.code===0){
                        ...
                        else $scope.vendorTemp.vid="null";
                    }else{
                        alert("获取vendorlist失败！");
                    }
    ...
    $scope.changeNav=function () {
        //先去掉其他已经被点击过的导航的颜色
        $(".active").toggleClass("active");
        $("#publishInfoNav").toggleClass("active");
    }();
    ...
    $scope.addressInit = function(_cmbProvince, _cmbCity, _cmbArea, defaultProvince, defaultCity, defaultArea)
    {
        ...
        {
            cmbAddOption(cmbProvince, $scope.provinceList[i].name, $scope.provinceList[i]);
        }
    ...
    $scope.addressInit('cmbProvince', 'cmbCity', 'cmbArea');
    ...
    $scope.displayTarget=function () {
        $scope.showTarget=!$scope.showTarget;
    };

    $scope.clickStatus_P=function (x) {
        x.selected = !x.selected;
        if(x.selected)
        {
            $scope.temp+=x.id+",";
        ...
        {
            $scope.temp=$scope.temp.replace(x.id+",","");
        }
        //alert($scope.temp);
        // alert($scope.temp);
    };
    $scope.submitChecked_P=function () {
        if($scope.targetEmpty)
            $scope.showTarget=false;
        else{
            $scope.act.target=$scope.temp.substr(0,$scope.temp.length-1);
            $scope.showTarget=false;
        }

    };
    //发布活动
    $scope.publishAct=function (act) {
        //将时间转换成时间戳存储
        act.msgAttrs.start_time=parseInt(new Date($scope.start_time).valueOf()/1000);
        act.msgAttrs.end_time=parseInt(new Date($scope.end_time).valueOf()/1000);
        ...
        {
            act.image=$scope.uploadImage;
        }
        //获取当前用户可操作vendorList列表的第一个vendor并发布活动
        var url="/service/admin/list/vendor";
        $http.post(url)
            .success(function(data){
                if(data.code===0){
                    if(data.list.length){
                        act.vid=data.list[0].id;
                        var url2="/service/publish/submit";
                        $http.post(url2,act)    //这里没有分号！！
                            .success(function(data){
                                if(data.code===0){
                                    alert("发布成功！");
                                    $location.path("/myActivity");
                        ...
                        error(function( ) {
                            console.log("PublishActivity:error");
                        });
                    }
                    else alert("vendorList获取结果为空!");
                }
                else{
                    alert("获取vendorlist失败!");
                }
    ...
    // 上传图片
    $scope.publishUploadImg=function(file) {
