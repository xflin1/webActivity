...
const TABLE = 'vendorServiceType';
var TABLE_FIELDS = ['vid','st']; //表字段
...
module.exports = {
     ...
     * @returns {*|bluebird}
     */
    getByVidSt:function(vid,st){
        return base.getBaseMulti(TABLE,{vid:vid,st:st});
    },
    /**
     * 获取vid的所有可操作serviceType
     * @param vid
     * @param [fields]
     * @returns {*|bluebird}
     */
    getServiceType:function(vid,fields){
        var query = sqlCmd.getVendorServiceType;
        var format;
        if(fields===undefined){
            query = query.replace('??','*');
            format = [vid];
